...
export const fetchResources = createAsyncThunk(
  'rbac/fetchResources',
  async () => {
    const response = await axios.get('http://localhost:8080/api/rbac/resources', {
      headers: {
...
export const fetchPermissions = createAsyncThunk(
  ...
  async () => {
    const response = await axios.get('http://localhost:8080/api/rbac/permissions', {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('access_token')}`,
      },
    });
...
export const fetchResourcePermissions = createAsyncThunk(
  ...
  async (roleId: number) => {
    const response = await axios.get(`http://localhost:8080/api/rbac/roles/${roleId}/resources`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('access_token')}`,
      },
    });
...
export const assignPermission = createAsyncThunk(
  ...
  async ({ roleId, resourceId, permissionId }: { roleId: number; resourceId: number; permissionId: number }) => {
    const response = await axios.post(
      `http://localhost:8080/api/rbac/roles/${roleId}/resources/${resourceId}/permissions/${permissionId}`,
      {},
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('access_token')}`,
...
export const fetchRoleUsers = createAsyncThunk(
  'rbac/fetchRoleUsers',
  async (roleId: number) => {
    const response = await axios.get(`http://localhost:8080/api/rbac/roles/${roleId}/users`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('access_token')}`,
      },
    });
...
export const assignUserToRole = createAsyncThunk(
  ...
  async ({ roleId, email }: { roleId: number; email: string }) => {
    const response = await axios.post(
      `http://localhost:8080/api/rbac/roles/${roleId}/assign-user`,
      { email },
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('access_token')}`,
...
export const removeUserFromRole = createAsyncThunk(
  ...
  async ({ userId, roleId }: { userId: number; roleId: number }) => {
    const response = await axios.delete(
      `http://localhost:8080/api/rbac/users/${userId}/roles/${roleId}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('access_token')}`,
        },
